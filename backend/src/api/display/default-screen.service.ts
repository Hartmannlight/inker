...
export class DefaultScreenService implements OnModuleInit {
  ...
  async generateDefaultScreen(
    ...
    try {
      ...
      const dithered = this.dither(grayBuffer.data, grayBuffer.info.width, grayBuffer.info.height);
  ...
  async ensureDefaultScreenForSize(
    ...
    try {
      ...
      if (bitDepth >= 4) {
        ...
        const dithered = this.dither(data, info.width, info.height);
        if (format === 'bmp') {
  ...
  private dither(data: Buffer, width: number, height: number): Buffer {
