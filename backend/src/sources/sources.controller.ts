...
export class SourcesController {
  constructor(
    private readonly commands: SourcesService,
    private readonly reads: SourceReadService,
  ) {}
  ...
  read(@Param('id') id: string) {
    return this.reads.read(id);
  }

  @Get(':id/snapshots/:snapshotId')
  snapshot(@Param('id') id: string, @Param('snapshotId') snapshotId: string) {
    return this.reads.snapshot(id, snapshotId);
  }
  ...
  refresh(@Param('id') id: string) {
    return this.commands.refresh(id);
  }
}
