...
async function bootstrapWorker() {
  ...
  const logger = createSafeLogger('worker');
  ...
  const server = createServer(async (request, response) => {
    if (request.url === '/live' && request.method === 'GET') {
      ...
      response.end(JSON.stringify({ status: stopping ? 'stopping' : 'alive', role: 'worker' })); return;
    }
...
void bootstrapWorker().catch(error => { logStartupFailure('worker', error); process.exit(1); });
